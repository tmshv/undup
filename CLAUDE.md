...
Tests live in `internal/scan/archives_test.go`. Run with `go test ./...`. `go vet ./...` and `go build ./...` are also useful static checks.

## Architecture

Single-purpose CLI. Four production files plus one test file:
