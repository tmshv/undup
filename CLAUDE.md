...
- `cmd/undup/main.go` — kong-based CLI parsing (`Root` positional, `-j / --workers` flag default 1, and `-m / --mode` flag with enum `archives,duplicates,all`). Dispatches to `runArchives`, `runDuplicates`, or `runAll`. `runAll` uses a `tee` helper to fan a single `scan.Walk` channel out to both detectors so the filesystem is traversed only once.
