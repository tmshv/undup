...
- `cmd/undup/main.go` — kong-based CLI parsing (`Root` positional, `-j / --workers` flag default 1, and `-m / --mode` flag with enum `archives,duplicates,all`). `-j` sizes both the walker fan-out and the duplicate detector's hash worker pool. Dispatches to `runArchives`, `runDuplicates`, or `runAll`. `runAll` uses a `tee` helper to fan a single `scan.Walk` channel out to both detectors so the filesystem is traversed only once; `tee` logs walk errors centrally and filters them out so each error prints once even with both detectors active.
